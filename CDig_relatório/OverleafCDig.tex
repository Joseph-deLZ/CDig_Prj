...
\textit{This work presents the design and real-time implementation of a digital temperature controller for a lumped-parameter thermal process actuated by PWM through an ESP32 micro\-controller. A gray-box, two-pole/one-zero model with transport delay (P2DZ) is identified from an open-loop PRBS experiment. An anti-windup PI controller is designed by the root-locus method to meet overshoot and peak-time specifications, and is then extended, in the state space, with an augmented observer that estimates and rejects the sinusoidal disturbance produced by a cooling fan. Continuous and discrete simulations and experimental results obtained on the ESP32 are reported and compared.}

\vskip 1mm% não altere esse espaçamento
\selectlanguage{brazil}
{\noindent \bf Resumo}: Este relatório descreve o projeto e a implementação, em tempo real, de um sistema de controle digital de temperatura. O processo térmico, a parâmetros concentrados, é aquecido por resistores acionados em PWM por um microcontrolador ESP32 e perturbado por um ventilador. A partir de um ensaio em malha aberta com sinal PRBS, identificou-se um modelo de dois polos, um zero e atraso de transporte (P2DZ). Na primeira etapa (Prj1) projetou-se, via Lugar Geométrico das Raízes, um controlador PI Anti-Windup discreto, atendendo às especificações de sobrepasso e tempo de pico e rejeitando perturbações constantes por partes. Na segunda etapa (Prj2), manteve-se o canal PI e acrescentou-se, no espaço de estados, um observador aumentado com modelo gerador de senoides, capaz de estimar e rejeitar a perturbação senoidal do ventilador. Apresentam-se as simulações (contínua e discreta) e os resultados experimentais coletados no ESP32, comparando-os com as especificações.
\end{abstract}
...
O controle de temperatura é um dos problemas mais recorrentes da engenharia de automação, presente em processos industriais, em sistemas de climatização (HVAC) e em equipamentos de laboratório. Processos térmicos são, em geral, lentos, dominados por uma ou mais constantes de tempo e por um atraso de transporte associado ao deslocamento do calor, o que os torna um excelente estudo de caso para técnicas de controle digital.

Este trabalho aborda o projeto e a implementação de um controlador digital para o processo térmico didático da disciplina de Controle Digital (ENE0167), atuado por modulação por largura de pulso (PWM) por meio de um microcontrolador ESP32. O trabalho divide-se em duas etapas complementares: \textbf{Prj1}, o projeto de um controlador PI Anti-Windup discreto via Lugar Geométrico das Raízes (LGR); e \textbf{Prj2}, a extensão do controle ao espaço de estados, com um observador aumentado capaz de rejeitar perturbações senoidais geradas por um ventilador. O modelo a parâmetros concentrados adotado segue os princípios fundamentais de transferência de calor descritos em \cite{Bauchspiess:2006}.

\subsection{Objetivos}
\noindent \textbf{Geral:} projetar e implementar, em tempo real no ESP32, um sistema de controle de temperatura que atenda a especificações dinâmicas e rejeite perturbações constantes e senoidais.

\noindent \textbf{Específicos:} (i) identificar experimentalmente o modelo do processo; (ii) projetar o PI Anti-Windup (Prj1) e o observador de perturbações senoidais (Prj2); (iii) discretizar e embarcar os controladores; (iv) validar em simulação e experimentalmente.

\subsection{Especificações de Projeto}

As especificações seguem o grupo 8 do roteiro \citep{Bauchspiess:CDig}, com ponto de operação P.O.$\,=60\,^{\circ}$C e frequência da perturbação senoidal $\omega_0 = 0{,}098$~rad/s (período $\approx 64$~s):
...
O processo térmico, ilustrado na Figura~\ref{fig:circuito}, é constituído por um sensor de temperatura LM35 aquecido por três resistores acionados em PWM através de um transistor BC548. A variável controlada $y$ é a temperatura $T_p$ do conjunto sensor/resistores; a variável manipulada $u$ é o \emph{duty-cycle} do PWM de aquecimento. Um segundo LM35 mede a temperatura ambiente $T_a$, medida ``longe'' da perturbação. Um pequeno ventilador (\emph{cooler}) acionado por um segundo canal PWM resfria o processo, gerando perturbações constantes e/ou senoidais. Filtros RC passa-baixa, dimensionados conforme o \textit{datasheet} do LM35 \citep{TI:LM35}, atenuam o ruído de medida. Um amplificador de instrumentação poderia ser usado para ajustar a faixa do sensor ($10\,$mV/$^{\circ}$C) à faixa do conversor A/D do ESP32 ($3{,}3$~V). No entanto, foi utilizada uma solução nativa do Espressif, a "analogSetPinAttenuation(pin, ADC\_0db)" que realiza um ajuste de resolução similar ao amplificador operacional com ajustes internos para as resistências do próprio ESP32.

O modelo a parâmetros concentrados parte da capacitância térmica do conjunto e dos fluxos de calor de aquecimento ($q_p$), do ambiente ($q_a$) e da perturbação ($q_w$), segundo a lei $q_i = K_i (T_i - T)$ para condução/convecção \citep{Bauchspiess:2006}. Trata-se de um modelo ``caixa-cinza'', pois associa o polo rápido ao aquecimento PWM e o polo lento ao acoplamento com $T_a$.

\begin{figure}[t]
\begin{center}
    \includegraphics[width=8.0cm]{figuras/foto_do_circuito.jpeg}
        \caption{Montagem experimental: \emph{protoboard} com ESP32 DevKit~v1, LM35, transistores BC548 e TIP31C, resistores de aquecimento (50$\Omega$, 1/4W) e uma fonte de 12~V/1,5~A para o estágio de potência.}
    \label{fig:circuito}
\end{center}
\end{figure}

\subsection{Materiais}
\textit{Protoboard}, ESP32 DevKit~v1 (chip USB--UART CP2102), 2$\times$LM35, 2$\times$BC548, \emph{cooler}, 3$\times$resistores ($\tfrac{1}{4}$~W) e filtro RC passa-baixa. A comunicação com o PC é feita por USB; o \textit{driver} CP210x foi instalado para o Windows~11 e a placa configurada no MATLAB como \texttt{ESP32-WROOM-DevKitV1}. Os sinais foram coletados pelo software \emph{CoolTerm}. O acionamento usa os pinos D16 (aquecedor) e D17 (\emph{cooler}); as leituras, os pinos D13 ($T_p$) e D4 ($T_a$). Observou-se, na montagem, um problema de \emph{sobrecorrente} em um dos BC548 ao acionar o \emph{cooler}, identificado após o cálculo da corrente para o dimensionamento (em $\tfrac{1}{4}$~W) dos resistores.
...
\begin{figure}[t]
 \begin{center}
  \includegraphics[width=8.4cm]{figuras/resultados/PRBS_resp.png}
...
com $T_a$. O modelo reproduz o conjunto de validação (não usado na estimação) com um ajuste
(\textit{fit}) de $90{,}8\,\%$, conforme a Figura~\ref{fig:fit}.

\begin{figure}[t]
 \begin{center}
  \includegraphics[width=8.4cm]{figuras/resultados/PRBS_resp_first_mins.jpeg}
 \caption{Validação do modelo: dados medidos (cinza) \emph{vs.} saída do modelo P2DZ (azul),
 com ajuste (\textit{fit}) de $90{,}8\,\%$ no conjunto de validação.}
 \label{fig:fit}
 \end{center}
\end{figure}
...
(Figura~\ref{fig:resp}) confirma $M_p \approx 45{,}5\,\%$ e $T_p \approx 30{,}4$~s, atendendo às
especificações; a leve excursão negativa inicial deve-se ao zero de fase não-mínima
introduzido pela aproximação de Padé.

\begin{figure*}[t]
\begin{center}
    \includegraphics[width=0.50\linewidth]{figuras/resultados/LGR_geogebra.jpeg}
    \hspace{0.01\linewidth}
    \includegraphics[width=0.42\linewidth]{figuras/resultados/LGR_crtl_sys_designer.png}
\end{center}
\caption{Projeto do PI no LGR. À esquerda, traçado no GeoGebra ($K_{\text{geo}}=0{,}3445$,
$A_{\text{geo}}=9{,}434$, de modo que $a = 1/A_{\text{geo}}$ e $K = K_{\text{geo}}\,A_{\text{geo}}$);
à direita, o LGR no \emph{Control System Designer} do MATLAB, com a região de projeto (cunha de
amortecimento e restrição de sobrepasso, em amarelo) e os polos de malha fechada em destaque.}
\label{fig:lgr}
\end{figure*}

\begin{figure}[t]
 \begin{center}
  \includegraphics[width=8.0cm]{figuras/resultados/temporal_resp.jpeg}
 \caption{Resposta ao degrau, em malha fechada, do PI projetado (modelo com Padé). Sobrepasso
 $M_p\approx 45{,}5\,\%$ no instante $t\approx 30{,}4$~s (amplitude de pico $1{,}45$), atendendo
 à especificação de tempo de pico.}
 \label{fig:resp}
 \end{center}
\end{figure}
...
\begin{figure*}[t]
\begin{center}
    \includegraphics[width=0.92\linewidth]{figuras/simulink-block-diagrams/diagrama_de_blocos_com_ZOH.jpeg}
\end{center}
...
\textbf{Prj2.} A Figura~\ref{fig:expprj2} apresenta o controle no espaço de estados com observador
senoidal, para duas taxas de amostragem: a de projeto, $T_s = T_d = 5$~s (coluna esquerda), e uma
mais rápida, $T_s = 1$~s (coluna direita). Em ambos os casos a saída segue a referência e rejeita o
degrau de perturbação; durante a perturbação senoidal, a perturbação estimada $\hat{e}_{\sin}$
(vermelho, gráficos inferiores) acompanha o acionamento cíclico do \emph{cooler} e a oscilação da
temperatura é reduzida em relação ao Prj1, confirmando a ação do observador. A amostragem a $1$~s
apresentou oscilação residual ligeiramente menor que a de $5$~s.

\begin{figure*}[t]
\begin{center}
    \includegraphics[width=0.47\linewidth]{figuras/PRJ2/PRJ2-y_real-sys_sampled_every_5s.png}\hfill
    \includegraphics[width=0.47\linewidth]{figuras/PRJ2/PRJ2-y_real-sys_sampled_every_1s.png}\\[1mm]
    \includegraphics[width=0.47\linewidth]{figuras/PRJ2/PRJ2-Us-sys_sampled_every_5s.png}\hfill
    \includegraphics[width=0.47\linewidth]{figuras/PRJ2/PRJ2-Us-sys_sampled_every_1s.png}
\end{center}
\caption{Resultado experimental do Prj2 com observador senoidal: amostragem a $T_s=5$~s (coluna
esquerda) e $T_s=1$~s (coluna direita). Acima: temperatura $Y_{real}$ (azul), \emph{setpoint}
(vermelho), $T_a$ (amarelo) e estado do \emph{cooler} (verde). Abaixo: sinal de controle $u$
(azul) e perturbação senoidal estimada $\hat{e}_{\sin}$ (vermelho).}
\label{fig:expprj2}
\end{figure*}
